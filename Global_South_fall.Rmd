...
all_words_only_1970 <- all_words_only_1970 %>%
  mutate(word_stem = wordStem(lemma))

seperate_tfidf_1970 <- left_join(all_words_only_1970 %>% count(id, lemma), all_words_only_1970 %>% count(id, lemma) %>% group_by(id) %>% summarize(`words in speech` = sum(n)))

full_tfidf_1970 <- seperate_tfidf_1970 %>%
...
govern_full_tfidf_1970 <- govern_1970_seperate_tfidf %>%
  bind_tf_idf(lemma, id, n) %>%
  filter(lemma %in% country_filter == FALSE) %>%
  arrange(desc(tf_idf)) %>%
  anti_join(stop_words, by = c("lemma" = "word"))

govern_full_tfidf_1970
```
